...
# Pre-Implementation Data Verification and Backup Script
# VerbumCare - Code Consistency, Security & Offline Capability
# 
# This script:
# 1. Documents all existing database tables and schemas
# 2. Creates a full database backup
# 3. Documents all existing data (counts, types, status)
# 4. Verifies the backup can be restored successfully
#
...
DB_CONTAINER="verbumcare-postgres"
...
if docker exec ${DB_CONTAINER} psql -U ${DB_USER} -d ${DB_NAME} -c "SELECT 1;" > /dev/null 2>&1; then
    ...
    echo -e "${RED}Please ensure Docker containers are running: docker-compose up -d${NC}"
    exit 1
fi
echo ""

echo -e "${YELLOW}Step 2: Documenting database schema...${NC}"
{
    echo "========================================="
...
echo -e "${YELLOW}Step 3: Documenting existing data...${NC}"
{
    echo "========================================="
    ...
    run_query_formatted "SELECT staff_id, username, full_name, role, facility_id FROM staff LIMIT 5;"
    echo ""
...
echo -e "${YELLOW}Step 4: Creating database backup...${NC}"
...
docker exec ${DB_CONTAINER} psql -U ${DB_USER} -c "CREATE DATABASE ${TEST_DB};" > /dev/null 2>&1
...
RESTORED_COUNT=$(docker exec ${DB_CONTAINER} psql -U ${DB_USER} -d ${TEST_DB} -t -A -c "SELECT COUNT(*) FROM patients;")
...
if [ "${ORIGINAL_COUNT}" == "${RESTORED_COUNT}" ]; then
    ...
    docker exec ${DB_CONTAINER} psql -U ${DB_USER} -c "DROP DATABASE ${TEST_DB};" > /dev/null 2>&1
    exit 1
fi
