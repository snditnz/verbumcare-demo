...
# VerbumCare Pre-Implementation Data Backup and Documentation Script (Docker Version)
...
if ! docker ps | grep -q "${CONTAINER_NAME}"; then
    ...
    echo -e "${YELLOW}Please start the database with: docker-compose up -d postgres${NC}"
    exit 1
fi
...
run_query() {
    local query="$1"
    docker exec -e PGPASSWORD="${DB_PASSWORD}" "${CONTAINER_NAME}" psql -U "${DB_USER}" -d "${DB_NAME}" -t -A -c "${query}"
}

# Function to run SQL query and return formatted result
run_query_formatted() {
    local query="$1"
    docker exec -e PGPASSWORD="${DB_PASSWORD}" "${CONTAINER_NAME}" psql -U "${DB_USER}" -d "${DB_NAME}" -c "${query}"
}

echo -e "${YELLOW}Step 1: Creating full database backup...${NC}"
docker exec -e PGPASSWORD="${DB_PASSWORD}" "${CONTAINER_NAME}" pg_dump -U "${DB_USER}" -d "${DB_NAME}" \
    --format=plain \
    --no-owner \
    --no-acl > "${BACKUP_FILE}"
...
if [ "${PATIENT_COUNT}" -gt 0 ]; then
    ...
    echo '```' >> "${DOCUMENTATION_FILE}"
    run_query_formatted "SELECT patient_id, mrn, family_name, given_name, date_of_birth, gender, room, bed, status FROM patients ORDER BY mrn LIMIT 10;" >> "${DOCUMENTATION_FILE}"
    echo '```' >> "${DOCUMENTATION_FILE}"
    echo "" >> "${DOCUMENTATION_FILE}"
...
docker exec -e PGPASSWORD="${DB_PASSWORD}" "${CONTAINER_NAME}" psql -U "${DB_USER}" -d postgres -c "DROP DATABASE IF EXISTS ${TEST_DB};" > /dev/null 2>&1
docker exec -e PGPASSWORD="${DB_PASSWORD}" "${CONTAINER_NAME}" psql -U "${DB_USER}" -d postgres -c "CREATE DATABASE ${TEST_DB};" > /dev/null 2>&1

# Restore backup to test database
echo -e "${BLUE}Restoring backup to test database...${NC}"
docker exec -i -e PGPASSWORD="${DB_PASSWORD}" "${CONTAINER_NAME}" psql -U "${DB_USER}" -d "${TEST_DB}" < "${BACKUP_FILE}" > /dev/null 2>&1
...
if [ $? -eq 0 ]; then
    ...
    TEST_PATIENT_COUNT=$(docker exec -e PGPASSWORD="${DB_PASSWORD}" "${CONTAINER_NAME}" psql -U "${DB_USER}" -d "${TEST_DB}" -t -A -c "SELECT COUNT(*) FROM patients;")
    ...
    docker exec -e PGPASSWORD="${DB_PASSWORD}" "${CONTAINER_NAME}" psql -U "${DB_USER}" -d postgres -c "DROP DATABASE ${TEST_DB};" > /dev/null 2>&1
